...
cat <<-EOF > "${TARGET_DIR}${CONFIG_SCRIPT}"
  ...
  # Chef
  /usr/bin/pacman -S --noconfirm ruby git yajl
  /usr/bin/curl https://aur.archlinux.org/cgit/aur.git/snapshot/package-query.tar.gz | tar xz -C /tmp
  /usr/bin/curl https://aur.archlinux.org/cgit/aur.git/snapshot/yaourt.tar.gz | tar xz -C /tmp
  chown -R vagrant:users /tmp/yaourt /tmp/package-query
  cd /tmp/package-query && /usr/bin/sudo -u vagrant /usr/bin/makepkg --needed -c --noconfirm -i package-query && cd -
  cd /tmp/yaourt && /usr/bin/sudo -u vagrant /usr/bin/makepkg --needed -c --noconfirm -i yaourt && cd -
  /usr/bin/sudo -u vagrant /usr/bin/yaourt -S --noconfirm chef-dk
...
echo '==> Entering chroot jail and configuring system'
